  ...
  _createClass(CockpitSDK, [{
    ...
    value: function image(assetId, options) {
      ...
      var width = opts.width,
          ...
          rest = _objectWithoutProperties(opts, ['width', 'height', 'quality', 'pixelRatio']);
    ...
    value: function imageSrcSet(assetId) {
      ...
      return widths.map(function (width) {
        if ((typeof width === 'undefined' ? 'undefined' : _typeof(width)) === 'object') return _this3.image(assetId, width) + ' ' + (width.srcSet || width.width ? width.width + 'w' : '');
